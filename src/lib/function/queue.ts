// based on https://github.com/galnir/Master-Bot/blob/ff9c924cd2745c51e9ca4b031f136c748baec9a2/src/lib/utils/queue/Queue.ts
...
export class Queue extends TypedEmitter<QueueEvents> {
	...
	add(songs: Addable | Array<Addable>, options: AddOptions = {}): number {
		...
		const requesterId = options.requester && getId(options.requester),
			toAdd = songs.map((song) => (song instanceof Song ? song : new Song(song, requesterId)));
